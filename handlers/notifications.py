...
async def get_notification_nav_keyboard(user_id: int):
    ...
    builder.button(text=texts["back_btn"], callback_data="notif_back", icon_custom_emoji_id="5260687119092817530")
...
async def start_notification_management(callback: types.CallbackQuery, state: FSMContext):
    ...
    builder.button(text=texts["add_new_btn"], callback_data="notif_add", icon_custom_emoji_id="5258260149037965799")
    builder.button(text=texts["main_menu_btn"], callback_data="main_menu", icon_custom_emoji_id="6042137469204303531", style="danger")
    builder.adjust(1)
...
async def view_notification(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
...
async def show_btn_input_screen(event, state: FSMContext, bot: Bot):
    ...
    texts = await get_locale(user_id)
    text = texts["enter_buttons"]

    builder = InlineKeyboardBuilder()
    builder.button(text=texts["skip_btn"], callback_data="notif_skip_btns")
    builder.button(text=texts["back_btn"], callback_data="notif_back")
    builder.button(text=texts["main_menu_btn"], callback_data="main_menu", style="danger")
    builder.adjust(1)
...
async def process_buttons(message: types.Message, state: FSMContext, bot: Bot):
    try: await message.delete()
    except: pass
    ...
    if data.get('is_editing'):
        ...
        await show_interval_selector(message, state, bot)

@router.callback_query(NotificationStates.ENTER_BUTTONS, F.data == "notif_skip_btns")
async def skip_buttons(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
    await state.update_data(custom_buttons=[])
    data = await state.get_data()
    if data.get('is_editing'):
...
async def show_interval_selector(event, state: FSMContext, bot: Bot):
    user_id = event.from_user.id if isinstance(event, types.CallbackQuery) else event.chat.id
    texts = await get_locale(user_id)
    text = texts["enter_interval"]
...
async def process_interval(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
    ...
    if val == "custom":
        user_id = callback.from_user.id
        texts = await get_locale(user_id)
        text = texts["custom_interval_title"]
        await safe_edit_text(callback, text, reply_markup=await get_notification_nav_keyboard(user_id), parse_mode=ParseMode.HTML, state=state)
...
async def show_notification_preview(event, state: FSMContext, bot: Bot):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text=texts["edit_title_btn"], callback_data="notif_edit_title")
    builder.button(text=texts["edit_text_btn"], callback_data="notif_edit_text")
    builder.button(text=texts["edit_buttons_btn"], callback_data="notif_edit_btns")
    builder.button(text=texts["edit_interval_btn"], callback_data="notif_edit_interval")
    builder.button(text=texts["edit_chat_btn"], callback_data="notif_edit_chat")

    status_label = texts["active"] if is_active else texts["paused"]
    builder.button(text=texts["toggle_status_btn"].format(status=status_label), callback_data="notif_toggle_status")

    builder.button(text=texts["save_btn"], callback_data="notif_confirm_save", style="success")
    builder.button(text=texts["main_menu_btn"], callback_data="main_menu", style="danger")
    builder.adjust(2, 2, 2, 1, 1)

    preview_text = texts["preview_header"] +         f"┣ <b>Title:</b> {html.escape(title)}\n" +         f"┣ <b>Text:</b> {html.escape(text)}\n" +         f"┣ <b>Interval:</b> {interval} min\n" +         texts["preview_footer"]
...
async def edit_title(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback, texts["enter_title"], reply_markup=await get_notification_nav_keyboard(user_id), parse_mode=ParseMode.HTML, state=state)
    await state.set_state(NotificationStates.ENTER_TITLE)
...
async def edit_interval(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await state.update_data(is_editing=True)
    await safe_edit_text(callback, texts["enter_interval"], reply_markup=get_interval_keyboard(), parse_mode=ParseMode.HTML, state=state)
    await state.set_state(NotificationStates.ENTER_INTERVAL)
...
async def confirm_save(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text=texts["main_menu_btn"], callback_data="main_menu", icon_custom_emoji_id="6042137469204303531")

    await safe_edit_text(callback, texts["save_success"], reply_markup=builder.as_markup(), parse_mode=ParseMode.HTML, state=state)
    await state.clear()
