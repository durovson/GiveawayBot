...
class Database:
    ...
    async def _ensure_connection(self):
        """Lightweight health check to ensure connection is alive."""
        if not self.client:
            await self.connect()
        # In a real scenario, we might want to do a simple ping here
        # but for now, we just ensure the client exists.
    ...
    async def add_participant(self, giveaway_id: int, user_id: int, username: Optional[str]) -> bool:
        ...
        try:
            ...
            if e.code == '23505' or getattr(e, 'status', None) == 409:
                return False
            logger.error(f"APIError adding participant: {e}")
            return False
        except Exception as e:
    ...
    async def try_start_giveaway_completion(self, giveaway_id: int) -> bool:
        """Atomic update to change status from active to processing."""
        if not await self._ensure_connection(): return False
        try:
            response = await self.client.table("giveaways")                 .update({"status": "processing"})                 .eq("id", giveaway_id)                 .eq("status", "active")                 .execute()
            return len(response.data) > 0
        except Exception as e:
            logger.error(f"Error starting giveaway completion: {e}")
            return False
    ...
    async def update_user_wallet(self, telegram_id: int, wallet_address: str):
        if not await self._ensure_connection(): return
        try:
